    ...
    if($ispost) {
        ...
        if(empty($error)) {
            ...
            $config = str_replace(array(
                '{db-server}', '{db-username}', '{db-password}', '{db-port}', '{db-name}', '{db-tablepre}', '{cfg-cookiepre}', '{cfg-authkey}', '{cfg-sitepath}'
            ), array(
                $db['server'], $db['username'], $db['password'], $db['port'], $db['name'], $db['prefix'], $cookiepre, $authkey, $sitepath
            ), $config);
            $dat = local_schema();
...
$config['common']['sitepath']           = '{cfg-sitepath}';
...
function tpl_install_db($error = '') {
            ...
            <div class="panel-body">
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-4">
                        ...
                        <input type="hidden" name="sitepath" value="" />
                    </div>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        function check(obj) {
            ...
            $(obj).html('正在执行安装');
            $(':hidden[name=sitepath]').val(location.pathname);
            return true;
        }
    </script>
EOF;
